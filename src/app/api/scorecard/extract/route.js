...
export async function POST(req) {
  ...
  try {
    ...
    // Reject payloads over ~10 MB of base64 (≈ 7.5 MB raw) — data minimisation
    // and protection against abuse. A Hole19 screenshot is typically < 1 MB.
    if (imageBase64.length > 10_000_000) {
      return NextResponse.json({ error: "Image too large (max ~7.5 MB). Please crop or compress it first." }, { status: 413 });
    }

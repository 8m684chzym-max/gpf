...
export async function GET(req) {
  ...
  let rows, filename;
  if (type === "road") {
    rows = [["Player", "Approved qualifying", "Handicap", "Qualified"]];
    buildRoadBoard(users, rounds, config).forEach((r) => rows.push([r.user.name, r.count, fmt(r.hcp), r.qualified ? "yes" : "no"]));

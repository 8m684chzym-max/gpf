...
import { ArrowUp, Calendar, ChevronDown, ChevronRight, Crown, Dices, Flag, History, Loader2, Medal, Plus, Trash2, Trophy, Users, Wifi, X } from "lucide-react";
...
export default function GamesPage() {
  ...
  const [live, setLive] = useState([]);        // active games from server
  const [history, setHistory] = useState([]);  // finished games
  ...
  // open game
  const [game, setGameState] = useState(null); // client shape from server
  ...
  // Boards.jsx still calls setGame(updater); we translate that into either a
  // local hole-navigation or a server write of the one hole that changed.
  ...
  // Points League — position points (4/3/2/1) from finished side games only.
  // Aggregated per member (by userId) or per guest (by name). Fully separate
  // from the Road to GPF Open — it never touches Stableford / handicaps.
  const league = useMemo(() => {
    ...
    for (const g of history) {
      ...
      if (!board.length || board[0].points <= 0) continue; // ignore unscored games
      board.forEach((p) => {
        const rank = board.filter((q) => q.points > p.points).length; // 0-based, ties share
        const pts = POS[rank] ?? 0;
        const key = p.userId || `guest:${p.name.toLowerCase()}`;
        const cur = agg.get(key) || { name: p.name, points: 0, games: 0, wins: 0 };
        cur.points += pts;
        cur.games += 1;
        if (rank === 0) cur.wins += 1;
        cur.name = p.name;
        agg.set(key, cur);
      });
    }
    ...
    return (
      <>
        ...
        <div className="stack" style={{ marginTop: 12 }}>
          <div className="ocr-note"><Wifi size={14} /> Live · everyone on game {game.code} sees this. You can browse holes without moving anyone else.</div>
          <button className="btn btn-ghost" onClick={finish}><Flag size={16} /> Finish game for everyone</button>
        </div>
      </>
    );
  }
    ...
    <div className="stack">
      ...
      {view === "league" ? (
        <>
          ...
          <p className="muted small">Position points from finished side games — 4 / 3 / 2 / 1 to the top four in each game. A season-long side ladder, completely separate from the Road to GPF Open.</p>
          {league.length === 0 ? (
          ...
          {live.map((g) => {
            ...
            return (
              <div key={g.id} className="card" style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <button className="btn btn-primary small-btn" onClick={() => openGame(g)}>Join</button>
              </div>
            );
          })}
        </>
      ...
      <div className="stack">
        {Object.values(GAMES).map((g) => (
          <button
            key={g.key}
            className="quick"
            style={type === g.key ? { borderColor: "var(--green-600)", outline: "2px solid var(--green-600)", outlineOffset: -1 } : {}}
            onClick={() => { setType(g.key); if (picked.length > g.max) setPicked(picked.slice(0, g.max)); }}
          >
            <GameIcon type={g.key} size={20} />
            <span style={{ flex: 1 }}>
              <span style={{ display: "block", fontWeight: 700 }}>{g.name}</span>
              <span className="muted small" style={{ fontWeight: 400 }}>{g.blurb}</span>
              <span className="muted small" style={{ fontWeight: 400 }}>{g.min === g.max ? `${g.min} players` : `${g.min}–${g.max} players`}</span>
            </span>
        ...
        <>
          <div className="sec-h" style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <span className="muted small" style={{ marginLeft: "auto", fontWeight: 400 }}>
              {picked.length}/{meta.min === meta.max ? meta.min : `${meta.min}–${meta.max}`}
            </span>
          </div>
            ...
            <div className="card" style={{ display: "grid", gap: 8 }}>
              {picked.map((p, i) => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  {meta.ordered && i > 0 && (
                    <button className="icon-btn" title="Move up (tee order)" onClick={() => moveUp(i)}><ArrowUp size={15} /></button>
                  )}
                  <button className="icon-btn" title="Remove" onClick={() => remove(p.id)}><X size={15} /></button>
                </div>
              ))}
          ...
          {availableMembers.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {availableMembers.map((m) => (
                <button key={m.id} className="btn btn-ghost small-btn" disabled={picked.length >= meta.max} onClick={() => toggleMember(m)}>
                  <Plus size={14} /> {m.name}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            />
            <button className="btn btn-ghost" onClick={addGuest} disabled={!guest.trim() || picked.length >= meta.max}><Plus size={16} /></button>
          </div>

          <button className="btn btn-primary" disabled={!canStart || creating} onClick={start} style={{ marginTop: 4 }}>
            {creating ? <Loader2 className="spin" size={17} /> : <Trophy size={17} />} Start {meta.name}
          </button>
          ...
          {history.map((g) => {
            ...
            return (
              <div key={g.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
                <button
                  className="quick"
                  style={{ width: "100%", border: "none", borderRadius: 0, background: "transparent" }}
